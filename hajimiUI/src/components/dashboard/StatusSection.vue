...
<template>
  <div class="info-box">
    ...
    <div class="api-key-stats-container">
      <h3 class="section-title" style="cursor: pointer; user-select: none;" @click="toggleApiKeyStats">
        API密钥使用统计 <span>{{ apiKeyStatsVisible ? '▼' : '▶' }}</span>
      </h3>
      <div v-if="apiKeyStatsVisible">
          ...
          <div v-for="(stat, index) in dashboardStore.apiKeyStats" :key="index" class="api-key-item">
            ...
            <div v-if="stat.model_stats && Object.keys(stat.model_stats).length > 0" class="model-stats-container">
              <div class="model-stats-header" @click="toggleModelFold(stat.api_key)">
                ...
                <span class="model-stats-toggle">{{ modelFoldState[stat.api_key] ? '▼' : '▶' }}</span>
              </div>
              
              <div v-if="modelFoldState[stat.api_key]" class="model-stats-list">
                <!-- 显示所有模型或前三个模型 -->
